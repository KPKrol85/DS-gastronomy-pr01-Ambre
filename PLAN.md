...
- [x] **PH4-01 — Complete the Lighthouse quality gate without weakening its contract** — **Priority:** High
  ...
  - [x] add the missing offline-page meta description and preload its two first-render fonts.
  - [x] verify Performance at or above `0.85` on all eight configured pages with the unchanged Lighthouse command.
  - [x] resolve the remaining `is-crawlable` failures on `offline.html` and `404.html` without making either utility page indexable, excluding pages, or weakening the `0.95` SEO threshold.
  - **Source:** `daily-AUDIT.md` — Lighthouse remediation follow-up and [P1-04].
