...
| SVG icon registry | all pages with the footer (contact rows + social links); `index.html` testimonials | `[data-icon]`; optional `data-icon-class`, `data-icon-title`; `data-icon-ready` guard | `js/icons.js` | `initIcons()` | Renders registered SVG icons into placeholder hosts; icons are decorative unless `data-icon-title` is set. Footer hosts are `.site-footer__contact-icon` and the icon-only `.site-button--social` links. |
